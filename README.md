...
- Float / Vector2D / Vector / Color / Rotator 변수 타입 지원
...
하지만 간단한 UI 애니메이션, 라이트 변화, 위치 이동 등의 "짧고 단순한 동작"마다,
Curve 에셋을 추가하거나 Timeline을 구성하는 것은 번거로울 수 있습니다.

이런 번거로움을 해결하고자 만든 것이 바로, Async Easing Action입니다.
...
| Loop Duration | 루프 1회에 걸리는 시간입니다. (v1.1) |
| Begin | 시작 값입니다. |
| End | 종료 값입니다. |
| Loop Count | 루프 횟수를 지정합니다. (v1.1) |
| Round Trip | 액션을 왕복 운동으로 전환합니다. (v1.1) |
| Max Update Rate | 초당 Update 최대 호출 횟수입니다. (v1.1) |
| Tickable When Paused | 게임이 일시정지되었을 때 실행 여부입니다. |

| Out Pin | Description |
| --- | --- |
| Update | 설정된 Update Rate에 따라 현재 보간 값과 진행률을 반환하는 이벤트입니다. |
| Completed | 보간이 정상적으로 끝났을 때 호출되는 이벤트입니다. |
| Canceled | 사용자가 액션을 취소했을 때 호출되는 이벤트입니다. |
| Value | 반환된 보간 값입니다. |
| Percent | 비동기 액션의 누적 진행도입니다. N회 루프 시 0.0 ~ N.0을 반환합니다. (v1.1) |
...
1. 이 저장소의 최신 릴리즈에서 'SimpleEasingFunction_1.1.zip'을 다운로드합니다.
   - 2026.05.31 기준 v1.1
...
## v1.1 업데이트 (2026.05.31)

- 버전 표기 규칙을 v1.0.0 형식에서 v1.1 형식으로 변경하였습니다.
- 반복 실행 기능을 추가했습니다. 'Loop Count' 횟수만큼 반복 실행됩니다.
  - 'Loop Count'가 0보다 작으면 무한히 루프합니다. 반드시 수동으로 액션을 종료해야 합니다.
- 반복 실행 기능이 추가됨에 따라, 'Duration'이 'Loop Duration'으로 변경되었습니다.
- 왕복 운동 옵션을 추가했습니다. 'Round Trip'이 true면 'Begin -> End -> Begin'을 하나의 Loop로 취급합니다.
- 'Max Update Rate'를 통해, 초당 Update 최대 호출 횟수를 지정할 수 있습니다.
  - 'Max Update Rate'가 0보다 작으면 매 Tick마다 Update를 호출됩니다.
  - 'Max Update Rate'가 0인 경우 Update를 호출하지 않습니다.
